...
class AddUserSkillForm(forms.Form):
    ...
    def __init__(self, agent, *args, **kwargs):
        ...
        if agent:
            ...
            self.fields["skill_type"].queryset = Ocp_Skill_Type.objects.all().exclude( Q(resource_type__isnull=False), Q(resource_type__context_agent__isnull=False), ~Q(resource_type__context_agent__id__in=context_ids) ).order_by('lft')
        ...
        try:
            ...
            if agent:
                ...
                if gen_et:
                    ...
                    self.fields["exchange_type"].queryset = Ocp_Record_Type.objects.filter(lft__gt=gen_et.lft, rght__lt=gen_et.rght, tree_id=gen_et.tree_id).exclude( Q(id__in=hidden_etids) ).order_by('lft') #Q(exchange_type__isnull=False), Q(exchange_type__context_agent__isnull=False), ~Q(exchange_type__context_agent__id__in=context_ids) )


                    self.fields["resource_type"].queryset = Ocp_Artwork_Type.objects.all().exclude( Q(resource_type__isnull=False), Q(resource_type__context_agent__isnull=False), ~Q(resource_type__context_agent__id__in=context_ids) ).order_by('lft') #| Q(resource_type__context_agent__isnull=True) )
                    self.fields["skill_type"].queryset = Ocp_Skill_Type.objects.all().exclude( Q(resource_type__isnull=False), Q(resource_type__context_agent__isnull=False), ~Q(resource_type__context_agent__id__in=context_ids) ).order_by('lft') #| Q(resource_type__context_agent__isnull=True) )
        ...
        try:
            ...
            if agent:
                ...
                if gen_et:
                    ...
                    self.fields["parent_type"].queryset = Ocp_Record_Type.objects.filter(lft__gt=gen_et.lft, rght__lt=gen_et.rght, tree_id=gen_et.tree_id).exclude( Q(exchange_type__isnull=False), ~Q(exchange_type__context_agent__id__in=context_ids) ).order_by('lft')

                    self.fields["resource_type"].queryset = Ocp_Artwork_Type.objects.all().exclude( Q(resource_type__isnull=False), ~Q(resource_type__context_agent__id__in=context_ids) | Q(resource_type__context_agent__isnull=True) ).order_by('lft')
                    self.fields["skill_type"].queryset = Ocp_Skill_Type.objects.all().exclude( Q(resource_type__isnull=False), ~Q(resource_type__context_agent__id__in=context_ids) | Q(resource_type__context_agent__isnull=True) ).order_by('lft')
...
class NewResourceTypeForm(forms.Form):
    ...
    def __init__(self, agent=None, *args, **kwargs):
        ...
        self.fields["unit_type"].queryset = Ocp_Unit_Type.objects.all().exclude(clas='faircoin').order_by('lft') #(Q(clas__contains='currency') | Q(parent__clas__contains='currency'))
        if agent:
            context_ids = [c.id for c in agent.related_all_agents()]
            if not agent.id in context_ids:
                context_ids.append(agent.id)
            ...
            self.fields["resource_type"].queryset = Ocp_Artwork_Type.objects.all().exclude( Q(resource_type__isnull=False), Q(resource_type__context_agent__isnull=False), ~Q(resource_type__context_agent__id__in=context_ids) ).order_by('lft')
            self.fields["related_type"].queryset = Ocp_Artwork_Type.objects.all().exclude( Q(resource_type__isnull=False), Q(resource_type__context_agent__isnull=False), ~Q(resource_type__context_agent__id__in=context_ids) ).order_by('lft')
...
class NewSkillTypeForm(forms.Form):
    ...
    def __init__(self, agent=None, *args, **kwargs):
        ...
        self.fields["unit_type"].queryset = Ocp_Unit_Type.objects.filter(lft__gt=time.lft, rght__lt=time.rght, tree_id=time.tree_id).order_by('lft') #all().exclude(clas='faircoin') #(Q(clas__contains='currency') | Q(parent__clas__contains='currency'))
        if agent:
            context_ids = [c.id for c in agent.related_all_agents()]
            if not agent.id in context_ids:
                context_ids.append(agent.id)
            ...
            self.fields["parent_type"].queryset = Ocp_Skill_Type.objects.all().exclude( Q(resource_type__isnull=False), Q(resource_type__context_agent__isnull=False), ~Q(resource_type__context_agent__id__in=context_ids) ).order_by('lft')
            self.fields["related_type"].queryset = Ocp_Artwork_Type.objects.all().exclude( Q(resource_type__isnull=False), Q(resource_type__context_agent__isnull=False), ~Q(resource_type__context_agent__id__in=context_ids) ).order_by('lft')
...
class WorkEventContextAgentForm(forms.ModelForm):
    ...
    def __init__(self, context_agent=None, *args, **kwargs):
        ...
        if context_agent:
            ...
            self.fields["ocp_skill_type"].queryset = Ocp_Skill_Type.objects.all().exclude( Q(resource_type__isnull=False), Q(resource_type__context_agent__isnull=False), ~Q(resource_type__context_agent__id__in=context_ids) ).order_by('lft')
        if kwargs.items():
